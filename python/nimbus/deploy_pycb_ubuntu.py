...
location='/opt/nimbus/ve/lib/python2.7/site-packages/pycb_test/'
...
call('cp %spycb-0.1-py2.7.egg %sbak/pycb-0.1-py2.7.egg_%s'%(location,location,backup_count))
cur.execute('insert into deploy_pycb (id) values (?)',(backup_count,))

conn.commit()
cur.close()
    
call('rm %spycb-0.1-py2.7.egg'%location)
call('zip -r pycb-0.1-py2.7.egg docs EGG-INFO pycb')
call('cp %spycb-0.1-py2.7.egg %s../'%(location,location))
