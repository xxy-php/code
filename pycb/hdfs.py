...
class HDFS(object):
    ...
    def put_object(self,bucketName,objectName,length):
        ...
        pycb.log(logging.INFO, "=====## msg is %s"%res.msg)
        conn.close()
        
        conn=httplib.HTTPConnection(redirect_host)
        pycb.log(logging.INFO, "=====## redirect_host is %s"%redirect_host)
        conn.connect()
    ...
    def get_md5(self):
        conn=httplib.HTTPConnection(self.name_node)
        conn.request("GET","/webhdfs/v1/%s/%s?op=GETFILECHECKSUM"%(self.bucketName,self.objectName))
        res=conn.getresponse()
        redirect_host,url=redirect_host_url(res)
        conn.close()
